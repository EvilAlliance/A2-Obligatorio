...
template<class K,class V, class X> class Heap {
    ...
    Heap(unsigned long long len, int(*comparepiority)(K, K), bool maxHeap, X* d, void(*inserthandler)(K, V*, X*) = NULL, void(*removehandler)(K, V, X*) = NULL){
    ...
    Heap(unsigned long long len, int(*comparepiority)(K, K), bool maxHeap, void(*inserthandler)(K, V*, X*) = NULL, void(*removehandler)(K, V, X*) = NULL){
        items = new Heap_Cell<K, V>[len]();
        comparePiority = comparepiority;
        expectedRoot = maxHeap ? 1 : -1;
        capacity = len;
    ...
    unsigned long long childsOfLetf(unsigned long long i){
        return 2 * i + 1;
    }

    unsigned long long childsOfRight(unsigned long long i){
        return 2 * i + 2;
    }
    ...
    void sink(unsigned long long i){
        ...
        while(cL < count){
            if( cR < count && comparePiority(items[cL].piority, items[cR].piority) != expectedRoot){
                cL = cR;
            }
    ...
    void levitate(unsigned long long i){
        ...
        while(i != 0 && comparePiority(items[(p = parentOf(i))].piority, items[i].piority) != expectedRoot ){
            Heap_Cell<K, V> temp = items[i];
            items[i] = items[p];
            items[p] = temp;
    ...
    void push(K piority, V val){
        ...
        if(insertHandler != NULL) insertHandler(piority, &val, data);
    }
    ...
    V pop(){
        ...
        items[0] = items[count];
        sink(0);
        if(removeHandler != NULL) removeHandler(c.piority, c.value, data);
    ...
    V* get(K k){
        for(unsigned long long i = 0; i < count; i ++){
            if(comparePiority(items[i].piority, k) == 0) return &items[i].value;
        }
        return NULL;
    }
    ...
    V remove(K k){
        for(unsigned long long i = 0; i < count; i ++){
            if(comparePiority(items[i], k) == 0){
                ...
                items[i] = items[--count];
                sink(i);
                if(removeHandler != NULL) removeHandler(k, c.value, data);
            }
        }
        return NULL;
    }
